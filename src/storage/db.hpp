...
struct NodeRuntimeStatusSnapshot {
  ...
  std::size_t outbound_target{0};
  bool advertised_endpoint_present{false};
  bool advertised_endpoint_likely_public{false};
  std::string advertised_endpoint;
  bool stun_enabled{false};
  bool stun_last_success{false};
  std::uint64_t stun_last_attempt_unix_ms{0};
  std::uint64_t stun_last_success_unix_ms{0};
  std::string stun_last_server;
  std::string stun_last_error_code;
  std::uint64_t stun_backoff_until_unix_ms{0};
  bool stun_endpoint_change_pending{false};
  ...
  std::string readiness_failure_codes_csv;
